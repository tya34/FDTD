...
## 2026-09-19：FDTD 上边界与监视器范围更新

- 九类结构（含 4V）的顶部空气余量均为 `212 um`：其余八类本次从 `12 um` 增加 `200 um`，4V 保留此前调整。Z 下边界仍在衬底顶面下方 `4 um`。
- 全部 315 个角度脚本的光源仍由对应 FDTD 入射边界计算位置，向内偏移 `2 um`；侧向光源的 Z 中心和跨度随 FDTD 区域变化。
- 五个场监视器的覆盖范围按主体卷曲结构包围盒确定，各方向额外留 `1 um`，不再按 FDTD 横向跨度的 90% 或顶部光源位置确定。原有截面位置、名称和频率设置保持不变。
- 监视器范围距 FDTD 的 X/Y 边界各 `49 um`，距顶边界 `211 um`；底部按主体实际最低点向下留 `1 um`，与 FDTD 底边界的间距为 `film_min_z - substrate_top_z + 3 um`。已进行脚本静态核查及写入后回读，未运行求解器；需重新执行建模脚本以应用修改。
