...
def source_position(source_lon_deg, source_lat_deg):
    ...
    base_azimuth = math.radians(225 - 0.5 * source_lat_deg)
    return (
        math.cos(lon) * math.cos(base_azimuth),
        math.cos(lon) * math.sin(base_azimuth),
        math.sin(lon),
    )
...
def build_source_block(source_lon_deg, source_lat_deg):
    ...
    lines = [
        ...
        f"# src_pos_x = {fmt_num(sx)}, src_pos_y = {fmt_num(sy)}, src_pos_z = {fmt_num(sz)}.",
